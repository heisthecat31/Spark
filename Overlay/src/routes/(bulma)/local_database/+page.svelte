<script>
	...
	function refresh() {
		...
		fetch('http://localhost:6724/api/db/jousts')
			.then((r) => r.json())
			.then((r) => (jousts = r))
			.then((r) => joustsByMatchTime());
		fetch('http://localhost:6724/api/db/events')
			.then((r) => r.json())
			.then((r) => (events = r));
	}
...
<div class="content" style="max-width: 80em; margin: auto;">
	...
	<div class="box" style="position: relative; top: -2em;">
		<button class="button" on:click={refresh} style="position: absolute; top:-3em; right: 0;"
			>Refresh</button
		>
