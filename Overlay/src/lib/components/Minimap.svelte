<script lang="ts">
	...
	function x_pos(z) {
		return (-z / 80 + 0.5) * 427 + 68;
	}

	function y_pos(x) {
		return (x / 32 + 0.5) * 165 + 5;
	}
</script>
	...
	{#if frame}
		{#if frame['teams'][0]['players']}
			{#each frame['teams'][0]['players'] as p}
				<div
					class="player blue positionable_element"
					class:possession={p['possession']}
					style="left:{x_pos(p['head']['position'][2])}px; top: {y_pos(
						p['head']['position'][0]
					)}px;"
				>
					<div />
		...
		{#if frame['teams'][1]['players']}
			{#each frame['teams'][1]['players'] as p}
				<div
					class="player orange positionable_element"
					class:possession={p['possession']}
					style="left:{x_pos(p['head']['position'][2])}px; top: {y_pos(
						p['head']['position'][0]
					)}px;"
		...
		{#if frame}
			<div
				id="disc"
				class="positionable_element"
				style="left:{x_pos(frame['disc']['position'][2])}px; top: {y_pos(
