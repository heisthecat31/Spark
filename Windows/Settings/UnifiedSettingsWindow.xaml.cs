...
namespace Spark
{
	...
	public partial class UnifiedSettingsWindow
	{
		...
		private async void DownloadTTSCacheFromTTS(object sender, RoutedEventArgs e)
		{
			...
			try
			{
				...
				{
					client.Headers.Add("User-Agent", "Spark-TTS-Updater");
					...
					// Find SparkTTSCache.zip asset
					string ttsCacheUrl = null;
					foreach (var asset in release.assets)
					{
						if (asset.name.ToString().Equals("SparkTTSCache.zip", StringComparison.OrdinalIgnoreCase))
						{
					...
					{
						new MessageBox("TTS Cache not found in the latest release.", "Error").Show();
						return;
					}

					// Download to AppData\Local\Temp\SparkTTSCache folder
					...
					string tempFile = Path.Combine(ttsCacheFolder, "SparkTTSCache.zip");
					...
					await client.DownloadFileTaskAsync(ttsCacheUrl, tempFile);
					progressDialog.Close();
					
					// Extract to the same folder (AppData\Local\Temp\SparkTTSCache)
					// First, extract to a temporary subfolder
					string extractTempFolder = Path.Combine(ttsCacheFolder, "temp_extract");
					if (Directory.Exists(extractTempFolder))
						Directory.Delete(extractTempFolder, true);
					
					ZipFile.ExtractToDirectory(tempFile, extractTempFolder);
					
					// Move all files from temp_extract to the main SparkTTSCache folder
					foreach (string file in Directory.GetFiles(extractTempFolder))
					{
						string destFile = Path.Combine(ttsCacheFolder, Path.GetFileName(file));
					...
					foreach (string dir in Directory.GetDirectories(extractTempFolder))
					{
						string dirName = Path.GetFileName(dir);
						string destDir = Path.Combine(ttsCacheFolder, dirName);
					...
					// Clean up
					Directory.Delete(extractTempFolder, true);
					File.Delete(tempFile); // Delete the zip file
			...
			finally
			{
				...
				button.Content = "Update TTS Cache";
			}
		}
	}
